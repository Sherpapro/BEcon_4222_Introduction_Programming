...
- `SELECT` is written first but evaluated last. This is why you can use a column alias in `ORDER BY` but not in `WHERE`.
...
- Close the connection when done.
- We usually convert the result to a panda DataFrame for further analysis in Python with `.df()`.
:::
:::
